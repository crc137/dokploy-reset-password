...
send_telegram_notification() {
    ...
    local response=$(curl -s -X POST "https://api.telegram.org/bot${TG_TOKEN}/sendMessage" \
        -d "chat_id=${TG_ADMIN}" \
        -d "text=$(printf '%s' "$message_escaped")" \
...
check_for_updates() {
    ...
    if [ "$1" = "--install" ] || [ "$1" = "--force" ] || [ "$1" = "-i" ] || [ "$1" = "-f" ]; then
        force_install=true
        log_message "INFO" "Force install mode enabled (manual installation requested)"
    fi
    ...
    if [ "$force_install" != "true" ]; then
        ...
        auto_update="true"
        log_message "INFO" "Force install mode: ignoring AUTOMATICALLY_CHECK_FOR_NEW_UPDATES setting"
    fi
...
<pre>$(echo "$changelog_text" | head -20)</pre>"
    ...
    if [ "$auto_update" = "true" ] || [ "$auto_update" = "1" ] || [ "$auto_update" = "yes" ] || [ "$auto_update" = "on" ] || [ "$force_install" = "true" ]; then
        if [ "$force_install" = "true" ]; then
            log_message "INFO" "Manual installation requested. Installing new version..."
            notification="$notification

<b>Manual installation</b> - Installing now..."
        else
...
if [ -d "\$SCRIPT_DIR" ]; then
    ...
    if [ "\$(ls -A "\$SCRIPT_DIR" 2>/dev/null | grep -v '^\.env\$')" ]; then
        ...
        [ -f "daily-password.sh" ] && cp "daily-password.sh" "\$BACKUP_DIR/" && log_info "  Backed up: daily-password.sh"
        ...
        [ -f "config.sh" ] && cp "config.sh" "\$BACKUP_DIR/" && log_info "  Backed up: config.sh"
...
if [ -d "\$SCRIPT_DIR" ]; then
    ...
    [ -f "daily-password.sh" ] && rm -f "daily-password.sh" && log_info "  Removed: daily-password.sh"
    [ -f "uninstall.sh" ] && rm -f "uninstall.sh" && log_info "  Removed: uninstall.sh"
    [ -f "update.sh" ] && rm -f "update.sh" && log_info "  Removed: update.sh"
    [ -f "config.sh" ] && rm -f "config.sh" && log_info "  Removed: config.sh"
...
download_with_retry() {
    ...
    if [ -z "\$url" ] || [ -z "\$output" ]; then
        log_error "download_with_retry: URL or output path is empty"
        return 1
    fi
    ...
    while [ \$attempt -le \$max_attempts ]; do
        ...
        if [ \$attempt -lt \$max_attempts ]; then
            ...
            if [ -z "\$delay" ] || ! [ "\$delay" -eq "\$delay" ] 2>/dev/null; then
                delay=2
            fi
...
verify_installation() {
    ...
    if [ -z "\$check_interval" ] || ! [ "\$check_interval" -eq "\$check_interval" ] 2>/dev/null; then
        check_interval=2
    fi
    ...
    while [ \$wait_time -lt \$max_wait ]; do
        ...
        sleep "\$check_interval" 2>/dev/null || sleep 2
        wait_time=\$((\$wait_time + \$check_interval))
        log_info "Waiting for port \${API_PORT} to be available... (\${wait_time}s/\${max_wait}s)"
    done
    
    log_error "Port \${API_PORT} is not listening after \${max_wait}s"
    return 1
}
...
show_help() {
    ...
    echo ""
    echo "Options:"
    echo "  --install, -i, --force, -f    Force installation of new version (ignores AUTOMATICALLY_CHECK_FOR_NEW_UPDATES setting)"
