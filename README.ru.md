...
# Сетевой доступ (по умолчанию: false = только localhost, безопасное
# значение по умолчанию). Включайте true только если сервис работает за
# TLS-прокси (например, Traefik, который уже использует Dokploy) - у этого
# API нет собственного TLS. См. раздел "Безопасность" ниже.
PUBLIC_BIND=false
...
- **По умолчанию только localhost.** API привязывается к `127.0.0.1`, если в `.env` не установлено `PUBLIC_BIND=true`. У сервиса нет собственного TLS, поэтому используйте SSH-туннель/VPN или разместите перед ним TLS-прокси (например, Traefik, который уже использует Dokploy), прежде чем включать `PUBLIC_BIND=true`.
